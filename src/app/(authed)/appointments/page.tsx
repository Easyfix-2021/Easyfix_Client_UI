...
/*
 * Committed Appointments — confirmed slots awaiting technician allocation
 * or in progress.
 *
 * Migrated from legacy ACD_APIs
 *   POST /api/clients/{clientId}/jobs?pageNo=…&pageSize=…&sortBy=job_id
 *   body: { flag: "allocated|unallocated|''", status: [0|1], clientSpocId }
 * (Angular UpcomingAppointmentsComponent — upcoming-appointments.component).
 *
 * Three tabs:
 *   1. Tx Unallocated  — flag=unallocated, status=[0]
 *      Confirmed orders awaiting technician allocation.
 *   2. Tx Allocated    — flag=allocated, status=[0]
 *      Confirmed orders awaiting tech app acknowledgement.
 *   3. Tx Ongoing Orders — no flag, status=[1]
 *      Confirmed scheduled orders proceeding as planned.
 *
 * Counts come from /api/client/appointments/counts (single round-trip,
 * scoped to SPOC's team to stay consistent with the list query).
 */
import Link from 'next/link';
import { useEffect, useMemo, useState } from 'react';
import {
  CalendarCheck, Search, ChevronLeft, ChevronRight,
  Filter, X, AlertTriangle, Eye, UserPlus, UserCheck, PlayCircle,
} from 'lucide-react';
...
type TabKey = 'unallocated' | 'allocated' | 'ongoing';

const TABS: Array<{
  key: TabKey; label: string; icon: React.ComponentType<{ className?: string }>;
  subtitle: string; flag: string; statuses: number[];
}> = [
  {
    key: 'unallocated',
    label: 'Tx Unallocated',
    icon: UserPlus,
    subtitle: 'Confirmed orders awaiting allocation of a technician.',
    flag: 'unallocated',
    statuses: [0],
  },
  {
    ...
    label: 'Tx Allocated',
    icon: UserCheck,
    subtitle: 'Confirmed orders awaiting acknowledgement from the technician on their app.',
    flag: 'allocated',
    statuses: [0],
  },
  {
    key: 'ongoing',
    label: 'Tx Ongoing Orders',
    icon: PlayCircle,
    subtitle: 'Confirmed orders that are scheduled and proceeding as planned.',
    flag: '',
    statuses: [1],
  },
];
...
export default function CommittedAppointmentsPage() {
  const spoc = useSpoc();
  const [tab, setTab] = useState<TabKey>('unallocated');
  ...
  // Tab counts — single endpoint with full filter set
  const countsPath = useMemo(() => {
    ...
    return `/appointments/counts?${p.toString().replace(/%2C/g, ',')}`;
  }, [applied, debouncedQ, spoc.id]);

  const countsRes = useFetch<{ txUnallocated: number; txAllocated: number; ongoingOrders: number }>(countsPath);
  const counts = countsRes.data ?? { txUnallocated: 0, txAllocated: 0, ongoingOrders: 0 };
  const countByTab: Record<TabKey, number> = {
    unallocated: counts.txUnallocated,
    allocated:   counts.txAllocated,
    ongoing:     counts.ongoingOrders,
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 inline-flex items-center gap-2">
            <CalendarCheck className="w-6 h-6 text-primary" /> Committed Appointments
          </h1>
          <p className="text-sm text-slate-500">{currentTab.subtitle}</p>
        </div>
      </div>

      {/* Tabs with icons + counts */}
      ...
      {/* Table — legacy ClientTable column shape:
          Job ID / Client Ref ID / Reference ID / Appointment / Customer /
          City / Category / Client SPOC / Created or Scheduled By / Age */}
        ...
        <table className="data-table">
          ...
          <tbody>
            ...
            {!loading && items.map((j) => {
              ...
              // Legacy logic: status=0 → createdBy, status=1+ → scheduledBy
              const lastActorName = tab === 'unallocated'
                ? (j.created_by_name || '—')
