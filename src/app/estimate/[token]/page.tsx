    ...
    return (
      <Shell>
        <div className="bg-white rounded-2xl shadow-xl p-10 text-center">
          ...
          <div className="mt-3 text-sm font-semibold text-slate-700">Loading estimate…</div>
        </div>
      </Shell>
    );
  }
  ...
  return (
    <Shell>
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-600">
              ...
              {info.easyfixer_name && <span className="text-slate-400">Raised by {info.easyfixer_name}</span>}
            </div>
          </div>
          {info.client_name && (
            <div className="text-right text-xs">
              <div className="text-slate-400">For</div>
              <div className="text-sm font-semibold text-slate-800">{info.client_name}</div>
            </div>
          )}
        </div>
        ...
        <div className="relative bg-slate-100" style={{ height: '70vh', minHeight: 460 }}>
          {!pdfLoaded && (
          ...
          {showReject && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-2">
              <div className="flex items-center justify-between">
                <div className="text-xs font-semibold text-slate-700">Why are you rejecting this estimate?</div>
                <button
                  type="button"
                  onClick={() => { setShowReject(false); setRejectReason(''); setRejectComment(''); setSubmitError(null); }}
                  className="w-7 h-7 rounded-full grid place-items-center text-slate-500 hover:text-slate-700 hover:bg-slate-200 transition"
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none pl-3 pr-9 py-2 text-sm border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-rose-400"
                >
          ...
          <div className="flex items-center gap-2 justify-end flex-wrap">
            ...
            {!showReject && (
              <button
                ...
                onClick={approve}
                disabled={submitting}
                className="px-4 py-2 text-sm font-semibold rounded-lg bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow hover:shadow-md inline-flex items-center gap-1.5 disabled:opacity-60"
              >
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-violet-50 flex flex-col">
      <header className="bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 text-white px-4 py-3 shadow">
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-3">
          <div className="font-bold text-lg">Estimate Approval</div>
...
function ErrorCard({
  ...
  const iconClass = tone === 'success' ? 'text-emerald-500' : 'text-rose-500';
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 sm:p-10 text-center">
      ...
      <p className="mt-2 text-sm text-slate-600 whitespace-pre-line">{subtitle}</p>
    </div>
  );
}
